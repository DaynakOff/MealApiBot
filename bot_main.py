...
from recipes_handler import router, OrderMeal
...
async def commands(message: types.Message):
    response = as_list(
        as_marked_section(
            Bold("Команды"),
            "/category_search_random - Получить случайные рецепты",
...
async def description(message: types.Message):
    await message.answer("Этот бот поможет вам найти рецепты блюд из различных категорий.")
